...
function update_ui(mcurrent_rowid, fPid) {
  ...
  if (fPid) {
    ...
    Relationship: $SPDXID CONTAINS DocumentRef-$ExtDocumentName:SPDXRef-$EscPrimaryPackageName
     */
    generate_spdx()
...
function generate_spdx() {
    ...
    var hkey = {}
    hinputs.map(i => hkey[hinputs[i].name] = hinputs[i].value)
    try {
    ...
    hinputs.map(i => {
		...
		pc[hinputs[i].name] = hinputs[i].value;
    })
